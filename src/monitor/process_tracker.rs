...
mod tests {
    ...
    fn test_pid_reuse_detected_as_new_process() {
        ...
        // First snapshot with PID 100 running Safari
        let initial = vec![
            create_test_process(100, "Safari", "/Applications/Safari.app", vec!["com.apple.security.network.client"]),
        ];
        tracker.detect_new_processes(create_snapshot(initial));
        
        // Safari terminates, PID 100 is reused by Chrome (simulated by removing Safari first)
        // In reality, the snapshot comparison only checks PID, not process identity
        let intermediate = vec![];  // Safari terminated
        tracker.detect_new_processes(create_snapshot(intermediate));
        
        // PID 100 reused by new process
        let reused = vec![
            create_test_process(100, "Chrome", "/Applications/Google Chrome.app", vec!["com.apple.security.network.server"]),
        ];
        let new_procs = tracker.detect_new_processes(create_snapshot(reused));
        
        assert_eq!(new_procs.len(), 1, "Reused PID should be detected as new process");
